...
import { IParams, ITeacher } from '../../../types';
import { Link } from 'react-router-dom';
import moment from 'moment';

type DataType = Partial<ITeacher>;

type DataIndex = keyof DataType;

const Teachers: React.FC = () => {
  const [filterParams, setFilterParams] = React.useState<IParams[]>([]);
  const { data: teacherData, isLoading } = useGetTeachersQuery(filterParams);
  ...
  const columns: TableColumnsType<DataType> = [
    ...
    },
    {
      ...
      render: (record) => (
        <Space>
          <Link to={`/admin/update-teacher-info/${record.key}`}>
  ...
  const data: DataType[] | undefined = teacherData?.data?.map(
    ({
      _id,
      name,
      gender,
